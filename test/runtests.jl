...
(x, f, gNorm, iter, optimal, tired, status, Stime) = steepestS(nlp, s=s, verbose=false)
...
reset!(nlp)
s = TStopping()
(x, f, gNorm, iter, optimal, tired, status, Stime) = steepestS(nlp, s=s, verbose=false, bk_max=2)
@printf("%-5s  %3d  %9.2e  %7.1e  %5d  %5d  %6d   %6d  %-20s  %7.3e\n",
        nlp.meta.name, nlp.meta.nvar, f, gNorm,
        nlp.counters.neval_obj, nlp.counters.neval_grad,
        nlp.counters.neval_hprod, iter, status, Stime)
