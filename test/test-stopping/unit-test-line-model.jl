@testset "Test Line-Search Stopping with LineModel" begin
  ...
  state = OneDAtX(0.0, f₀ = obj(pb, 0.), g₀ = grad(pb, 0.))
  @test state.x == 0.0
  @test !isnan(state.f₀)
  @test !isnan(state.g₀)
  @test isnan(state.fx)
  ...
  meta = StoppingMeta(optimality_check = (pb, lsatt) -> lsatt.fx - lsatt.f₀ - lsatt.g₀ * lsatt.x * τ₀,
                      tol_check = (a,b,c) -> 0.0, 
  ...
  stp2 = NLPStopping(pb, OneDAtX(0., f₀ = obj(pb, 0.), g₀ = grad(pb, 0.)), main_stp = NLPStopping(nlp), optimality_check = armijo)

  @test update_and_start!(stp2, fx = 0.0, gx = 0.0)
  @test update_and_stop!(stp2, fx = 0.0, gx = 0.0)

  #Check an OR condition
  lsatt = OneDAtX(0.0, f₀ = obj(pb, 0.), g₀ = grad(pb, 0.), (NaN, NaN))
  @test isnan.(lsatt.current_score) == (true, true)
  @test lsatt.x == 0.0

  #Check an AND condition
  lsatt = OneDAtX(0.0, f₀ = obj(pb, 0.), g₀ = grad(pb, 0.), [Inf, Inf])
  τ₀ = 1e-4 #Armijo parameter
  τ₁ = 0.99 #Wolfe parameter
  meta = StoppingMeta(optimality_check = (pb, lsatt) -> vcat(lsatt.fx - lsatt.f₀ - lsatt.g₀ * lsatt.x * τ₀,
                                                             abs(lsatt.gx) - τ₁ * abs(lsatt.g₀)),
                      tol_check = (a,b,c) -> 0.0, 
                      tol_check_neg = (atol, rtol, opt0) -> -Inf)
  src  = StopRemoteControl(resources_check = false)
